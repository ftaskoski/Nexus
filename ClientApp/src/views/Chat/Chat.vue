...
import { getFriend, getMessages, sendMessage } from './store'
      ...
      <template #content>
        <div ref="scrollContainer" class="flex flex-col h-96 overflow-y-auto p-4" @scroll="handleScroll">
          <div v-if="loading" class="flex justify-center items-center h-10">
            <Icon
              v-if="loading" icon="loading" size="20px" class="animate-spin text-black mb-4" fill="currentColor"
              color="none"
            />
          </div>
          <div v-for="msg in messages" :key="msg.id" class="mb-4">
            <div
              class="max-w-xs md:max-w-md p-3 rounded-lg" :class="[
                msg.receiverId === friendId ? 'ml-auto bg-blue-500 text-white rounded-br-none' : 'mr-auto bg-gray-100 text-gray-800 rounded-bl-none',
              ]"
            >
              <p>{{ msg.content }}</p>
              <div class="text-xs mt-1" :class="[msg.receiverId === friendId ? 'text-blue-100' : 'text-gray-500']">
      ...
      <template #footer>
        <div class="border-t p-4 ">
          <div class="flex items-center gap-2">
            <Input
              id="chat-input" v-model="message" placeholder="Type a message..." class="flex-1"
              @keyup.enter="sendMsg" @input="handleTyping"
            />
            <div class="flex justify-end">
              <Button type="primary" size="m" @click="sendMsg">
