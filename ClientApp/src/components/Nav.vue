<template>
    <div class="fixed left-0 top-0 h-screen w-64 border-r border-gray-200 bg-white p-4">
      ...
      <nav class="space-y-1">
        <router-link
          ...
          :to="item.to || '/'"
          :key="item.id"
          class="group flex w-full items-center rounded-lg px-3 py-3 text-sm font-medium transition-all duration-200 ease-in-out"
          :class="[
            activeItem === item.id 
        ...
        >
          <svg 
            class="mr-4 h-6 w-6 transition-transform duration-200 ease-in-out"
            :class="[
